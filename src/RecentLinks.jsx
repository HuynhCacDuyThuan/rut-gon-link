...
const RecentLinks = () => {
  ...
  useEffect(() => {
    const fetchLinks = async () => {
      try {
        const response = await fetch('https://flask-backend-1-yhfi.onrender.com/all');
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://flask-backend-1-yhfi.onrender.com/update1${shortUrl}`, {
        method: 'POST',
        headers: {
            ...
            <div key={index} className="card shadow-sm p-3 mb-2">
              <div className="d-flex align-items-start">
                <div className="flex-grow-1">
                  ...
                  <a
  href={`https://flask-backend-1-yhfi.onrender.com${link.short_url}`}
  target="_blank"
  rel="nofollow"
  className="align-middle"
>
  <strong className="text-break">{`https://flask-backend-1-yhfi.onrender.com${link.short_url}`}</strong>
</a>

                  </div>
                  {/* Button to Copy URL */}
                  <CopyToClipboard text={`https://flask-backend-1-yhfi.onrender.com${link.short_url}`}>
      ...
      <Modal show={showEditModal} onHide={() => setShowEditModal(false)}>
        ...
        <Modal.Body>
          ...
          <InputGroup className="mb-3">
            ...
            <FormControl
              value={`https://flask-backend-1-yhfi.onrender.com${editLink}`}
              readOnly
            />
          </InputGroup>
