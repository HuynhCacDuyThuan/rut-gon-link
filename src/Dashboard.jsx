...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const response = await fetch('https://flask-backend-5-28db.onrender.com/stats');  // Update the URL if needed
  ...
  useEffect(() => {
    const fetchDailyStats = async () => {
      try {
        const response = await fetch('https://flask-backend-5-28db.onrender.com/stats/daily');  // Update URL if needed
        const data = await response.json();
