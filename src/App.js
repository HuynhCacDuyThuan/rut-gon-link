...
const App = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('https://flask-backend-1-yhfi.onrender.com/shorten', {
        method: 'POST',
        headers: {
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`https://flask-backend-1-yhfi.onrender.com/update/${shortenedUrl.split('/')[1]}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const copyToClipboard = () => {
    navigator.clipboard.writeText(`https://flask-backend-1-yhfi.onrender.com${shortenedUrl}`);
